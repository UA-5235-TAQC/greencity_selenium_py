...
def test_verify_edit_news_form_fields_visibility(edit_news_page_with_language: EditNewsPage):
    """
    Test to validate all fields and interactions in the Edit News form.

    Steps:
    1. Set window size and switch language.
    2. Verify Title field behavior: append, prepend, remove characters.
    3. Verify Tags field: selection, deselection, limits.
    4. Verify Image Upload component: visibility, error messages, cropper buttons, change image.
    5. Verify Source field: value, validation message.
    6. Verify Content field: visibility, toolbar, counter, messages, append/prepend/remove content.
    7. Verify Author field.
    8. Verify Post Date field.
    9. Verify Publish/Edit, Preview, Cancel buttons.
    10. Verify Cancel modal functionality.
    11. Verify News Preview page.
    12. Verify Edit News after returning from preview.
    """
    ...
    with allure.step("Verify Title field initial state and modifications"):
        with allure.step("Check initial state of Title field"):
        ...
        with allure.step("Append text to Title"):
        ...
        with allure.step("Prepend text to Title"):
        ...
        with allure.step("Remove first and last characters from Title"):
        ...
        with allure.step("Enter final test Title"):
    ...
    with allure.step("Verify Tags field behavior"):
    ...
    with allure.step("Verify Image Upload component behavior"):
    ...
    with allure.step("Verify Source field validation"):
    ...
    with allure.step("Verify Content field behavior"):
        content = edit_news_page.content_component

        with allure.step("Check initial state of Content field"):
        ...
        with allure.step("Append text to Content field"):
            content.enter_content_not_clear(data["to_append"])
            assert content.is_content_valid(), "Content is invalid after append"
            check.equal(content.get_content_counter_text(), data["content_counter_after_append"])
            check.equal(content.get_actual_content_length(), data["content_length_after_append"])
            check.equal(content.get_content_text(), data["test_content"] + data["to_append"],
                        "Content should be appended")

        with allure.step("Prepend text to Content field"):
            content.prepend_content(data["to_prepend"])
            assert content.is_content_valid(), "Content is invalid after prepend"
            check.equal(content.get_content_counter_text(), data["content_counter_after_prepend"])
            check.equal(content.get_actual_content_length(), data["content_length_after_prepend"])
            check.equal(content.get_content_text(),
                        data["to_prepend"] + data["test_content"] + data["to_append"],
                        "Content should be prepended")

        with allure.step("Remove first and last characters from Content field"):
            content.remove_last_content_chars(data["remove_last"])
            content.remove_first_content_chars(data["remove_first"])
            check.equal(content.get_content_counter_text(), data["content_counter_after_remove"])
            check.equal(content.get_actual_content_length(), data["content_length_after_remove"])
            check.equal(content.get_content_text(), data["test_content"],
                        "Content should return to initial test content")

        with allure.step("Clear Content field"):
        ...
        with allure.step("Enter final Content text"):
            test_string = data["base_title"] + " " + data["test_content"]
            content.enter_content(test_string)
            check.equal(content.get_content_counter_text(), data["content_counter_final"])
            check.equal(content.get_actual_content_length(), data["content_length_final"])
            check.equal(content.get_content_text(), test_string)
    ...
    with allure.step("Verify buttons visibility and labels"):
    ...
    with allure.step("Verify Cancel modal behavior"):
        with allure.step("Open Cancel modal"):
        ...
        with allure.step("Verify Cancel modal buttons are visible"):
        ...
        with allure.step("Verify Cancel modal text content"):
            check.equal(
        ...
        with allure.step("Close Cancel modal and verify page state"):
    ...
    with allure.step("Verify News Preview page"):
    ...
    with allure.step("Edit news after returning from Preview page"):
