...
class BaseClient:
    ...
    def post(self, path: str, json: Optional[dict] = None, data: Optional[dict] = None, files: Optional[dict] = None,
             params: Optional[dict] = None, headers: Optional[dict] = None) -> Response:
        """ Execute POST request. """
        request_headers = self.prepare_request(headers)
        return requests.post(f"{self.base_api_url}{path}", headers=request_headers, json=json, data=data, files=files,
            params=params)
    ...
    def put(self, path: str, json_put: Optional[dict] = None, files: Optional[dict] = None,
        ...
        """Execute PUT request."""
        request_headers = self.prepare_request(headers)
        return requests.put(f"{self.base_api_url}{path}", headers=request_headers, json=json_put, files=files)

    def patch(self, path: str, json_patch=None, files: Optional[dict] = None, headers: Optional[dict] = None,
              params: dict = None) -> Response:
        """Execute PATCH request."""
        request_headers = self.prepare_request(headers)
        return requests.patch(f"{self.base_api_url}{path}", headers=request_headers, json=json_patch, files=files,
            params=params)
    ...
    def attach_images_to_multipart(self, files: list, control_name: str, image_paths: Optional[List[str]]) -> None:
        ...
        for path in image_paths:
            if path:
                ...
                files.append((control_name, (file_name, open(path, "rb"), mime_type)))
