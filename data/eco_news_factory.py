...
class EcoNewsDtoFactory:
    ...
    CONTENT_EN: str = (
        "The Saxe-Goldstein hypothesis is a prediction in archaeology "
    ...
    CONTENT_UK: str = (
        "Осип Тадейович Назарук (1883 — 1940) — український громадський і "
    ...
    def create_news_en(self) -> EcoNewsRequest:
        ...
        return EcoNewsRequest(
            title=self.TITLE_EN,
            text=self.CONTENT_EN,
            short_info=self.SHORT_INFO_EN,
            source=self.SOURCE_EN,
            tags=EcoNewsTag.get_en(self.TEST_TAGS)
        )
    ...
    def create_news_uk(self) -> EcoNewsRequest:
        ...
        return EcoNewsRequest(
            title=self.TITLE_UK,
            text=self.CONTENT_UK,
            short_info=self.SHORT_INFO_UK,
            source=self.SOURCE_UK,
            tags=EcoNewsTag.get_ua(self.TEST_TAGS)
        )

    def update_dto_en(self) -> UpdateEcoNewsRequest:
        """Create UpdateEcoNewsRequest in English"""
        return UpdateEcoNewsRequest(
            id=self.eco_news_id,
            title=self.TITLE_EN,
